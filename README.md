...
- 生成考勤表：根据汇总表和考勤表模板批量生成员工考勤表，保留模板格式、图片、宏和固定文本；下半年模板可选配独立的深夜加班汇总表。
...
深夜加班模板会自动识别新增列及右移后的员工编号、假期汇总、备注和辅助列。深夜加班汇总表按员工编号、护照号、姓名依次匹配，每天填写的数字会原值写入个人考勤表深夜加班列，合计对应 `TOTAL HOURS（合计）`。
